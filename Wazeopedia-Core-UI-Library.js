...
(function() {
    const WazeopediaUI = (function() {
        ...
        const publicApi = {
            ...
            toggleDropdown: function(dropdownContentElement) {
                ...
                if (!isCurrentlyShown) dropdownContentElement.classList.add('wz-show');
            },
            insertTextAtCursor: function(textarea, text, cursorConfig = {}) {
                ...
                if (cursorConfig.select) {
                    textarea.selectionStart = start;
                    textarea.selectionEnd = start + text.length;
                } else if (typeof cursorConfig.position === 'number') {
                    textarea.selectionStart = textarea.selectionEnd = start + cursorConfig.position;
                } else {
            ...
            applyHeadingFormatting: function(textarea, level, text = '') {
                ...
                let coreText = selectedText ? `${wzhTagOpen}${selectedText}${wzhTagClose}` : `${wzhTagOpen}${wzhTagClose}`;
                let textToInsert = markdownPrefix + coreText;
                ...
                const cursorPosition = selectedText ? textToInsert.length : (textToInsert.length - wzhTagClose.length);
            ...
            showTocGuideModal: function(textarea, tocTemplates) {
                ...
                modal.innerHTML = `
                    ...
                    <select id="wz-toc-template-select"></select>
                    <div id="wz-toc-outline-display"></div>
                    <div class="wz-modal-buttons">
                        <button id="wz-toc-insert-btn" class="wz-confirm">Insertar Esquema</button>
                ...
                Object.keys(tocTemplates).forEach(key => {
                    ...
                    option.value = key; option.textContent = tocTemplates[key].title;
                ...
                const updateDisplay = () => {
                    ...
                    if (!template) return;
                    display.innerHTML = '';
                    template.structure.forEach(line => {
                        const numberMatch = line.match(/^([\d\.]+)/);
                        if (!numberMatch) return;
                        const level = (numberMatch[1].match(/\d+/g) || []).length;
                        const indent = '  '.repeat(Math.max(0, level - 1));
                        const item = document.createElement('div');
                        item.className = 'wz-toc-item'; item.innerHTML = indent + line;
                        item.onclick = () => {
                ...
                modal.querySelector('#wz-toc-insert-btn').onclick = () => {
                    const template = tocTemplates[select.value];
                    if (!template) return;
                    const textToInsert = template.structure.map(line => {
                        const text = line.replace(/^[\d\.]+\s*/, '').trim();
                        const level = (line.match(/^([\d\.]+)/)[1].match(/\d+/g) || []).length;
                        return `${'#'.repeat(level)} [wzh=${level}]${text}[/wzh]`;
                    }).join('\n\n');
                    publicApi.insertTextAtCursor(textarea, textToInsert);
                    publicApi.closeAllModals();
                };
                select.onchange = updateDisplay;
                modal.querySelector('#wz-toc-close-btn').onclick = publicApi.closeAllModals;
                updateDisplay();
    ...
    console.log('Wazeopedia Core UI Library 6.2.0 loaded.');
})();
