...
(function () {
    ...
    window.WazeopediaUI = (function () {
        ...
        Object.assign(publicApi, {
            ...
            toggleDropdown: function (dropdownContentElement) { if (!dropdownContentElement) return; const isCurrentlyShown = dropdownContentElement.classList.contains('wz-show'); closeAllDropdowns(); if (!isCurrentlyShown) dropdownContentElement.classList.add('wz-show'); },
            ...
            createFormattingToolbar: function (textarea, buttonsToShow = ['bold', 'italic', 'link']) {
                ...
                const buttonActions = {
                    ...
                    'link': {
                        label: '🔗', title: 'Hipervínculo', action: () => {
                            ...
                            modalContent.querySelector('.wz-confirm').onclick = () => { const url = urlInput.value.trim(); const title = titleInput.value.trim(); if (url) { const linkText = title || url; if (selectedText) { publicApi.insertTextAtCursor(textarea, '', { wrap: [`[`, `](${url})`] }); } else { publicApi.insertTextAtCursor(textarea, `[${linkText}](${url})`); } } closeThisModal(); };
    ...
    console.log('Wazeopedia Core UI Library v9.0.0.2 loaded.');
})();
