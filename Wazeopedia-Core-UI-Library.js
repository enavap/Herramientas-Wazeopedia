...
(function() {
    ...
    const WazeopediaUI = (function() {
        ...
        const publicApi = {
            ...
            createFormattingToolbar: function(textarea, buttonsToShow = ['bold', 'italic', 'link']) { const toolbar = document.createElement('div'); toolbar.className = 'wz-format-toolbar'; const buttonActions = { 'bold': { label: 'B', title: 'Negrita', action: () => publicApi.insertTextAtCursor(textarea, '', { wrap: ['**', '**'] }) }, 'italic': { label: 'I', title: 'Cursiva', style: 'font-style: italic;', action: () => publicApi.insertTextAtCursor(textarea, '', { wrap: ['*', '*'] }) }, 'link': { label: '🔗', title: 'Hipervínculo', action: () => { publicApi.closeAllModals(); const selectedText = textarea.value.substring(textarea.selectionStart, textarea.selectionEnd); const overlay = document.createElement('div'); overlay.className = 'wz-modal-overlay'; overlay.style.zIndex = '2100'; const modalContent = document.createElement('div'); modalContent.className = 'wz-modal-content'; modalContent.innerHTML = `<h3>Insertar hipervínculo</h3><div class="wz-modal-scrollable-content"><label for="wz-link-url">Copia una URL o escribe para buscar temas</label><input type="text" id="wz-link-url" placeholder="https://ejemplo.com"><label for="wz-link-title">Título opcional</label><input type="text" id="wz-link-title" value="${selectedText}"></div><div class="wz-modal-buttons"><button class="wz-cancel">${t('cancel')}</button><button class="wz-confirm">${t('accept')}</button></div>`; overlay.appendChild(modalContent); document.body.appendChild(overlay); const urlInput = modalContent.querySelector('#wz-link-url'); const titleInput = modalContent.querySelector('#wz-link-title'); urlInput.focus(); modalContent.querySelector('.wz-confirm').onclick = () => { const url = urlInput.value.trim(); const title = titleInput.value.trim(); if (url) { const linkText = title || url; const textToInsert = `[${linkText}](${url})`; publicApi.insertTextAtCursor(textarea, textToInsert, { select: true }); } publicApi.closeAllModals(); }; modalContent.querySelector('.wz-cancel').onclick = () => publicApi.closeAllModals(); publicApi.setupModalEscape(overlay); } }, 'quote': { label: '“', title: 'Cita', action: () => publicApi.insertTextAtCursor(textarea, '> ', { position: 2 }) }, 'emoji': { label: '😀', title: 'Emojis', action: () => publicApi.showModal("Usa el selector de emojis de tu sistema (Tecla Windows + .)", "alert") } }; buttonsToShow.forEach(key => { if (buttonActions[key]) { const config = buttonActions[key]; const button = document.createElement('button'); button.innerHTML = config.label; button.title = config.title; if (config.style) button.style.cssText = config.style; button.onclick = (e) => { e.preventDefault(); config.action(); }; toolbar.appendChild(button); } }); textarea.parentNode.insertBefore(toolbar, textarea); textarea.style.borderTopLeftRadius = '0'; textarea.style.borderTopRightRadius = '0'; textarea.style.borderTop = 'none'; return toolbar; }
        };
        loadStyles();
        return publicApi;
    })();
