...
(function() {
    ...
    window.WazeopediaBlocks = (function() {
        ...
        const ensureProperSpacing = (currentText, newBlockText, position, relativeBlockData) => {
        ...
        const generateBodyContentAndTitleParams = (cleanedPostTitleForDisplay) => {
        ...
        const generateFullForumBlock = (bodyContentTextForTemplate, urlEncodedNewTopicTitle) => {
            const bodyParamUnencoded = Content.FORUM_BLOCK.BODY_TEMPLATE(bodyContentTextForTemplate);
            const newTopicUrl = `https://www.waze.com/discuss/new-topic?category=spain-usuarios-y-editores/wazeopedia-es/4779&title=WAZO%20-%20${urlEncodedNewTopicTitle}&body=${encodeURIComponent(bodyParamUnencoded)}`;
            return `---
...
${Content.FORUM_BLOCK.LINK_TEXT_TEMPLATE.replace('{{NEW_TOPIC_URL}}', newTopicUrl)}`;
        ...
        const parseExistingTitleBlock = (editorText) => {
            if (!editorText.startsWith(Content.TITLE_BLOCK.TOC_MARKER)) return null;
            const wzBoxStartIndex = editorText.indexOf(Content.TITLE_BLOCK.WZBOX_START);
            if (wzBoxStartIndex === -1) return null;
            const wzBoxEndIndex = editorText.indexOf(Content.TITLE_BLOCK.WZBOX_END, wzBoxStartIndex);
            if (wzBoxEndIndex === -1) return null;
            const content = editorText.substring(wzBoxStartIndex + Content.TITLE_BLOCK.WZBOX_START.length, wzBoxEndIndex);
            const titleMatch = content.match(/\[center\]\[wzh=1\](.*?)\[\/wzh\]\[\/center\]/);
            const title = titleMatch ? titleMatch[1].trim() : "";
            let statusKey = "aprobado", forumUrl = "";
            for (const key in Content.TITLE_BLOCK.STATUS_OPTIONS) {
                if (content.includes(Content.TITLE_BLOCK.STATUS_OPTIONS[key].text.split('***')[1])) {
                    statusKey = key;
                    if (Content.TITLE_BLOCK.STATUS_OPTIONS[key].requiresUrl) {
            ...
            return { title, statusKey, forumUrl, startIndex: 0, endIndex: wzBoxEndIndex + Content.TITLE_BLOCK.WZBOX_END.length };
        };

        const parseExistingIntroductionBlock = (editorText) => {
            const fullHeaderSearchIndex = editorText.indexOf(Content.INTRO_BLOCK.HEADER);
            if (fullHeaderSearchIndex === -1) return null;
            const contentStartAfterFullHeader = fullHeaderSearchIndex + Content.INTRO_BLOCK.HEADER.length;
            ...
            let endOfBlockIndex = editorText.indexOf("\n\n---", actualMainTextStartIndex);
            ...
            const noteBlockPattern = "\n\n" + Content.INTRO_BLOCK.NOTE_PREFIX;
            ...
            return { mainText, noteText, additionalText, hasNote, hasAdditional, startIndex: fullHeaderSearchIndex, endIndex: endOfBlockIndex + "\n\n---".length };
        };
        ...
        const isValidBioDate = (dateText) => { const text = dateText.trim(); return dateRegexDayMonthYear.test(text) || dateRegexMonthYear.test(text) || dateRegexYear.test(text); };
        const getBioEntryPrefix = (dateText) => { const text = (dateText || "").trim().toLowerCase(); if (dateRegexDayMonthYear.test(text)) return "El "; if (dateRegexMonthYear.test(text)) return "En "; if (dateRegexYear.test(text)) return "En el año "; return ""; };
        
        const parseExistingBiographyBlock = (editorText) => {
            ...
            if (blockContent) {
                blockContent.split('\n').forEach(line => {
                    ...
                    if (linkMatch) {
                        ...
                        let foundMatch = null;
                        if (dmyMatch) { foundMatch = dmyMatch[1]; }
                        else if (myMatch) { foundMatch = myMatch[1]; }
                        else if (yMatch) { foundMatch = yMatch[1]; }
                        
                        if (foundMatch && core.startsWith(foundMatch)) {
                            dateText = foundMatch;
    ...
    console.log('Wazeopedia Blocks Library v9.0.2 loaded.');
})();
